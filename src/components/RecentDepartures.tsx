  ...
  useEffect(() => {
    async function fetchDepartures() {
      try {
        ...
        setDepartures((data.departures ?? []).slice(0, 5));
      } catch (error) {
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-gray-800">Recent Departures</h3>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-10 text-center px-4">
          <ArrowUpCircle size={32} className="text-gray-300 mb-2" />
        ...
        <div className="divide-y divide-gray-50">
          ...
          {departures.map((departure) => (
            <div key={departure.id} className="grid grid-cols-4 gap-2 px-5 py-3 hover:bg-gray-50 transition-colors items-center">
              <span className="text-sm font-medium text-gray-700 truncate">{departure.name}</span>
              <StatusBadge status={departure.status} />
              <span className="text-sm text-gray-600">{departure.quantity}</span>
              <div className="flex items-center gap-1 text-xs text-gray-400">
                <Clock size={11} />
                <span>{new Date(departure.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
          ))}
        </div>
